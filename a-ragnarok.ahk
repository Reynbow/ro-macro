...
HudAlwaysOnTop := IniBool("HUD", "AlwaysOnTop", true)
global ElementOverlayBoxPx := 50
boxRaw := Trim(IniRead(SettingsFile, "ElementOverlay", "BoxPx", "50"))
if RegExMatch(boxRaw, "^\d+$") {
    ElementOverlayBoxPx := Integer(boxRaw)
    if ElementOverlayBoxPx < 24
        ElementOverlayBoxPx := 24
    else if ElementOverlayBoxPx > 400
        ElementOverlayBoxPx := 400
}
WVGui := ""
...
GamePingLastMs := ""
GamePingLastHost := ""
GamePingRefreshIntervalMs := 60000
...
GetStateJSON() {
    ...
    s := '{"needsOnboarding":' JsonBool(OnboardingNeeded)
        ...
        . ',"hudAlwaysOnTop":' JsonBool(HudAlwaysOnTop)
        . ',"elementOverlayBoxPx":' ElementOverlayBoxPx
...
ElementOverlayCreateGui(def) {
    global ElementOverlayBoxPx
    sz := ElementOverlayBoxPx
    ...
    numPt := Max(10, Round(sz * 36 / 50))
    lblPt := Max(6, Round(sz * 10 / 50))
    g.SetFont("s" numPt " bold c" numColor, "Segoe UI")
    g.AddText(Format("x0 y0 w{1} h{1} Center 0x201 Background{2}", sz, def.bg), String(def.num))
    g.SetFont("s" lblPt " bold c" def.fg, "Segoe UI")
    ; Foreground on top; transparent gaps show the large digit. 0x201 = centered; +0x100 = SS_NOTIFY for drag.
    elTxt := g.AddText(Format("x0 y0 w{1} h{1} Center 0x201 +BackgroundTrans +0x100", sz), def.label)
...
ElementOverlayRectTouchesWorkArea(x, y, w := unset, h := unset) {
    global ElementOverlayBoxPx
    if !IsSet(w)
        w := ElementOverlayBoxPx
    if !IsSet(h)
        h := ElementOverlayBoxPx
    cnt := MonitorGetCount()
    Loop cnt {
...
ElementOverlayClampToWork(&x, &y, w := unset, h := unset) {
    global ElementOverlayBoxPx
    if !IsSet(w)
        w := ElementOverlayBoxPx
    if !IsSet(h)
...
ElementOverlayApplyGameFocusVisibility() {
    global ElementOverlayGuis, ElementOverlayBoxPx
    ...
    for elName, g in ElementOverlayGuis {
        ...
        if IsSet(px) {
            try {
                if !DllCall("IsWindowVisible", "ptr", g.Hwnd)
                    g.Show(Format("x{} y{} w{} h{} NA", px, py, ElementOverlayBoxPx, ElementOverlayBoxPx))
            }
        }
    }
    ElementOverlayRaiseAll()
...
ElementOverlayDefaultPos(stackIdx) {
    ...
    gap := ElementOverlayBoxPx + 6
    x := gx + 10
    y := gy + gh + 8 + stackIdx * gap
...
SetElementOverlayCoordsBridge(jsonStr) {
    global ElementOverlayGuis, SettingsFile, ElementOverlayBoxPx
    ...
    x := Integer(data["x"])
    y := Integer(data["y"])
    ElementOverlayClampToWork(&x, &y, ElementOverlayBoxPx, ElementOverlayBoxPx)
    ...
    if ElementOverlayGuis.Has(el) {
        ...
        if IsObject(g)
            try g.Move(x, y, ElementOverlayBoxPx, ElementOverlayBoxPx)
    }
    ElementOverlayPushCoordsToWeb()
    return GetStateJSON()
}
...
SaveElementOverlayIni() {
    global SettingsFile, ElementOverlayEnabled, ElementOverlayBoxPx
    for name in ["Earth", "Wind", "Water", "Fire", "Ghost", "Shadow", "Holy"]
        IniWrite ElementOverlayEnabled[name] ? 1 : 0, SettingsFile, "ElementOverlay", name
    IniWrite ElementOverlayBoxPx, SettingsFile, "ElementOverlay", "BoxPx"
}


RefreshElementOverlayGuis() {
    global ElementOverlayGuis, ElementOverlayEnabled, ElementOverlayBoxPx
    DestroyElementOverlayGuis()
    if !ElementOverlayAnyEnabled()
        return
    sz := ElementOverlayBoxPx
    ...
    for def in ElementOverlayMeta() {
        ...
        try {
            if ElementOverlayReadSavedPos(def.name, &px, &py) {
                ElementOverlayClampToWork(&px, &py, sz, sz)
                g.Show(Format("x{} y{} w{} h{}", px, py, sz, sz))
            } else {
                pos := ElementOverlayDefaultPos(stackIdx)
                stackIdx++
                g.Show(Format("x{} y{} w{} h{}", pos.x, pos.y, sz, sz))
            }
        } catch Error as e {
...
SpamKeyPhysicallyDown(keyName) {
    kw := KeyWaitKeyName(keyName)
    if kw = " "
        return GetKeyState("Space", "P")
    return GetKeyState(kw, "P")
}


; StopAllSpam / StopKeySpam clear these to break out of RunSpamLoopForSlot early.
SetSpamSlotHeld(slot, v) {
    global QHeld, WHeld, EHeld, RHeld, ZHeld, XHeld, CHeld, VHeld
...
; No SetTimer: release polling never runs while another thread SendKeyPresses the same key (that
; made WaitSpamKeyReleasedStable + timer unreliable — spam stuck on or cutting out).
RunSpamLoopForSlot(slot, key, spamBaseDelay) {
    global MacrosEnabled, TargetProcess

    upStart := 0
    loop {
        if !SpamSlotHeld(slot)
            break
        if !(MacrosEnabled && IsKeyEnabled(slot))
            break
        if SpamKeyPhysicallyDown(key) {
            upStart := 0
            ...
            SendKeyPress(key)
            Sleep GetRandomDelay(spamBaseDelay)
        } else {
            if upStart = 0
                upStart := A_TickCount
            else if (A_TickCount - upStart >= 50)
                break
            Sleep 10
        }
    }
...
SaveSettingsBridge(jsonStr) {
    ...
    if data.Has("hudAlwaysOnTop")
        HudAlwaysOnTop := data["hudAlwaysOnTop"] ? true : false
    if data.Has("elementOverlayBoxPx") {
        v := ToIntSafe(data, "elementOverlayBoxPx", ElementOverlayBoxPx)
        if v < 24
            v := 24
        else if v > 400
            v := 400
    ...
    ApplyElementOverlaySettingsFromJson(data)
    SaveElementOverlayIni()
    IniWrite HudAlwaysOnTop ? 1 : 0, SettingsFile, "HUD", "AlwaysOnTop"
...
ApplyHudAlwaysOnTop() {
    global WVGui, HudAlwaysOnTop
    if !IsObject(WVGui)
        return
    try WinSetAlwaysOnTop(HudAlwaysOnTop ? 1 : 0, "ahk_id " WVGui.Hwnd)
}
...
BuildWebGui() {
    global WVGui, HudX, HudY, HudAlwaysOnTop
    indexHtml := "
    (
<!DOCTYPE html>
<html lang="en">
...
.settings-hud-top-field .settings-hud-top-head {
  display: flex;
  flex-direction: row;
...
.settings-hud-top-field .settings-hud-top-copy {
  flex: 1 1 auto;
  min-width: 0;
}
.settings-hud-top-field .settings-hud-top-copy .help {
  margin-top: 6px;
}
.settings-hud-top-field .settings-inline-label {
  display: block;
  font-size: 13px;
  font-weight: 600;
  color: var(--text);
}
.settings-hud-top-field .settings-hud-top-switch {
  flex: 0 0 auto;
  padding-top: 1px;
}
      ...
      <div class="settings-field settings-hud-top-field">
        <div class="settings-hud-top-head">
          <div class="settings-hud-top-copy">
            <span class="settings-inline-label">Macro HUD always on top</span>
            <div class="help">PIN keeps the HUD above other windows. Element tiles on the game use their own layering and are not affected.</div>
          </div>
          <div class="settings-hud-top-switch">
            <button type="button" class="macro-main-switch is-on" id="setHudAlwaysOnTopSwitch" onclick="toggleHudAlwaysOnTopSwitch()" title="Pin HUD above other windows" role="switch" aria-checked="true">
              <span class="switch-hit switch-hit-left" aria-hidden="true"></span>
              <span class="switch-hit switch-hit-right" aria-hidden="true"></span>
              <span class="macro-switch-thumb" aria-hidden="true"></span>
              <span class="macro-switch-words">
                <span class="macro-switch-word macro-sw-on">PIN</span>
                <span class="macro-switch-word macro-sw-off">OFF</span>
              </span>
            </button>
          </div>
...
function syncHudAlwaysOnTopSwitchFromState(state) {
  var el = document.getElementById('setHudAlwaysOnTopSwitch');
  if (!el) return;
  var on = state.hudAlwaysOnTop !== false;
...
window.roSuppressSettingsPersist = false;
window._persistSettingsTimer = null;
...
function buildSettingsPayloadFromDom() {
  ...
  var payload = {
    hudAlwaysOnTop: !!(swTop && swTop.classList.contains('is-on')),
    elementOverlayBoxPx: boxPx,
...
function persistSettingsNow() {
  if (window.roSuppressSettingsPersist) return;
  try {
    var cv = typeof chrome !== 'undefined' ? chrome : window.chrome;
    if (!cv || !cv.webview || !window.ahk || !ahk.global) return;
    var payload = buildSettingsPayloadFromDom();
    ahk.global.SaveSettingsBridge(JSON.stringify(payload)).then(function(json) {
      ...
      alert('Could not save settings: ' + (err && err.message ? err.message : String(err)));
    });
  } catch (e0) {}
}

function schedulePersistSettings(ms) {
  if (window.roSuppressSettingsPersist) return;
  clearTimeout(window._persistSettingsTimer);
  window._persistSettingsTimer = setTimeout(function() {
    window._persistSettingsTimer = null;
    persistSettingsNow();
  }, ms == null ? 400 : ms);
}
...
function initSettingsAutoSave() {
  ...
  if (!root || root._roSettingsAutoSave) return;
  root._roSettingsAutoSave = true;
  ...
  root.addEventListener('change', function(e) {
    ...
    schedulePersistSettings(60);
  });
  root.addEventListener('input', function(e) {
    if (window.roSuppressSettingsPersist) return;
    if (ignoreTarget(e.target)) return;
    schedulePersistSettings(480);
  });
}
...
function populateSettingsFromState(state) {
  ...
  var bx = document.getElementById('setElementOverlayBoxPx');
  if (bx) {
    var n = state.elementOverlayBoxPx != null ? Number(state.elementOverlayBoxPx) : 50;
    if (!isFinite(n) || n < 24) n = 24;
    if (n > 400) n = 400;
    bx.value = String(Math.round(n));
  }
...
ahk.global.GetStateJSON().then(function(json) {
  ...
  refreshElementOverlayCoordsFromHost();
    ...
    hudOpt := HudAlwaysOnTop ? "+AlwaysOnTop" : "-AlwaysOnTop"
    WVGui := WebViewGui(hudOpt " -Caption", "RO Macro")
...
MacroHotkeyPressed(slot, *) {
    ...
    SetSpamSlotHeld(slot, true)
    RunSpamLoopForSlot(slot, key, spamMs)
    SetSpamSlotHeld(slot, false)
}
...
; SpamSlotHeld is set only during RunSpamLoopForSlot (same thread as SendKeyPress). StopAllSpam /
; StopKeySpam clear the flag so the loop exits without racing a timer.
SpamSlotHeld(slot) {
    global QHeld, WHeld, EHeld, RHeld, ZHeld, XHeld, CHeld, VHeld
